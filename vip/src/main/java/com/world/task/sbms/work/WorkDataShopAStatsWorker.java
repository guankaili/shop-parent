...
import com.world.task.sbms.thread.WorkDataShopAStatsThread;
 ...
 **/
public class WorkDataShopAStatsWorker extends Worker {

    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public WorkDataShopAStatsWorker(String name, String des) {
        super(name, des);
    }
    ...
    public void run() {
        ...
        if (workFlag) {
            ...
            try {
                ...
                //店铺进本数据查询
                List<Bean> shopBeans = Data.Query("shop_member", sqlShopBase, null, WorkDataShopAStats.class);
                if (StringUtil.isNotEmpty(shopBeans)) {
                    List<WorkDataShopAStats> ShopBaseR = ObjectConversion.copy(shopBeans, WorkDataShopAStats.class);
                    //获取全国排名 和总数
                    String sqlRank = " SELECT " +
                            ...
                            "ORDER BY " +
                            "countryRank DESC ";
                    List<Bean> shopRank = Data.Query("shop_member", sqlRank, null, WorkDataShopAStats.class);
                    if (StringUtil.isNotEmpty(shopRank)) {
                        List<WorkDataShopAStats> scanRankR = ObjectConversion.copy(shopRank, WorkDataShopAStats.class);
                        //基础数据与排名合并
                        ...
                        String sqlRankByBF = " SELECT " +
                                ...
                                "ORDER BY " +
                                " areaRank DESC ";
                        List<Bean> shopRankByBF = Data.Query("shop_member", sqlRankByBF, null, WorkDataShopAStats.class);
                        if (StringUtil.isNotEmpty(shopRankByBF)){
                            List<WorkDataShopAStats> scanRankByBFR = ObjectConversion.copy(shopRankByBF, WorkDataShopAStats.class);
                            ShopBaseR.forEach(item1 -> {
                        ...
                        String sqlRankByNF = " SELECT " +
                                ...
                                "ORDER BY " +
                                " areaRank DESC ";
                        List<Bean> shopRankByNF = Data.Query("shop_member", sqlRankByNF, null, WorkDataShopAStats.class);
                        if (StringUtil.isNotEmpty(shopRankByNF)){
                            List<WorkDataShopAStats> scanRankByNFR = ObjectConversion.copy(shopRankByNF, WorkDataShopAStats.class);
                            ShopBaseR.forEach(item1 -> {
                    ...
                    for(WorkDataShopAStats workDataShopAStats : ShopBaseR){
                        //业务处理线程
                        WorkDataShopAStatsThread workDataShopAStatsThread = new WorkDataShopAStatsThread(workDataShopAStats,countDownLatch);
                        executorService.submit(workDataShopAStatsThread);
                    }
    ...
    public static void main(String[] args) {
        WorkDataShopAStatsWorker workDataShopAStatsWorker = new WorkDataShopAStatsWorker("WorkDataShopAStatsWorker", "门店统计首页统计月统计 数据同步");
        workDataShopAStatsWorker.run();
    }
