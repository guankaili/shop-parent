...
import com.world.task.sbms.thread.WorkDataShopAddAStatsThread;
import com.world.task.sbms.thread.WorkDataShopSignBuyAStatsThread;
 ...
 * @desc 签约、抢购 柱状图 数据获取
 **/
public class WorkDataShopSignBuyAStatsWorker extends Worker {

    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public WorkDataShopSignBuyAStatsWorker(String name, String des) {
        super(name, des);
    }
        ...
        if (workFlag) {
            ...
            try {
                String sql = "SELECT " +
                        ...
                        "GROUP BY " +
                        "t.dealer_cm_id ";
                List<Bean> beans = Data.Query("shop_member", sql, null, WorkDataShopSignBuyAStats.class);
                if (StringUtil.isNotEmpty(beans)){
                    List<WorkDataShopSignBuyAStats> scanList = ObjectConversion.copy(beans, WorkDataShopSignBuyAStats.class);
                    List<WorkDataShopSignBuyAFromStats> list = fromData(scanList);

                    //构建线程池
                    //当提交的任务数量为1000的时候，会开辟20个线程数
                    ExecutorService executorService = Executors.newFixedThreadPool(10);
                    CountDownLatch countDownLatch = new CountDownLatch(list.size());
                    for(WorkDataShopSignBuyAFromStats workDataShopSignBuyAFromStats : list){
                        //业务处理线程
                        WorkDataShopSignBuyAStatsThread workDataShopSignBuyAStatsThread = new WorkDataShopSignBuyAStatsThread(workDataShopSignBuyAFromStats,countDownLatch);
                        executorService.submit(workDataShopSignBuyAStatsThread);
                    }
     ...
     * @return
     */
    private List<WorkDataShopSignBuyAFromStats> fromData(List<WorkDataShopSignBuyAStats> scanList) {
        List<WorkDataShopSignBuyAFromStats> signBay = new ArrayList<WorkDataShopSignBuyAFromStats>();
        for (WorkDataShopSignBuyAStats bean : scanList) {
            WorkDataShopSignBuyAFromStats fromBean = new WorkDataShopSignBuyAFromStats();
    ...
    public static void main(String[] args) {
        WorkDataShopSignBuyAStatsWorker workDataShopSignBuyAStatsWorker = new WorkDataShopSignBuyAStatsWorker("workDataShopSignBuyAStatsWorker", "门店统计 签约、抢购门店列表 数据同步");
        workDataShopSignBuyAStatsWorker.run();
    }
}
