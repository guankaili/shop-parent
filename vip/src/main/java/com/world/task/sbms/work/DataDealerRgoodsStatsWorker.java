...
import com.world.task.sbms.thread.DataDealerRgoodsStatsThread;
 ...
 */
public class DataDealerRgoodsStatsWorker extends Worker {
    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public DataDealerRgoodsStatsWorker(String name, String des) {
        super(name, des);
    }
    @Override
    public void run(){
        log.info("数据模块年退货量统计:【年退货量统计】开始");
        if (workFlag) {
            workFlag = false;
            try {
                //1、获取年出货量统计
                String sql = "SELECT MONTH (t.create_datetime) m,t.dealer_code dealerCode,t.dealer_name dealerName," +
                        ...
                        "WHERE DATE_FORMAT(t.create_datetime,'%Y')=DATE_FORMAT(CURDATE(),'%Y') AND t.scan_type = 2 " +
                ...
                if(!CollectionUtils.isEmpty(list)){
                    ...
                    log.info("任务名称【" + "dataDealerRgoodsStatsWorker" + "】开始执行.此轮需要执行【" + list.size() + "】条数据任务");
                    //1、清除表数据
                    String trancateSql = " truncate table data_dealer_rgoods_stats ";
                    ...
                    groupByDealerCode.forEach((k,v) -> {
                        ...
                        groupByBrandCode.forEach((k1,v1) -> {
                            //业务处理线程
                            DataDealerRgoodsStatsThread dataDealerRgoodsStatsThread = new DataDealerRgoodsStatsThread(v1,countDownLatch);
                            executorService.submit(dataDealerRgoodsStatsThread);
                        });
                    });
                    ...
                    log.info("任务名称【" + "dataDealerRgoodsStatsWorker" + "】此轮没有需要执行的数据.");
                }

            } catch (Exception e) {
                log.error("年退货量统计失败", e);
            } finally {
                workFlag = true;
            }
        }else {
            log.info("数据模块年退货量统计:【年退货量统计】上一轮任务尚未结束，本轮不需要运行");
        }
    }

    public static void main(String[] args) {
        DataDealerRgoodsStatsWorker workHomeStatsWorker = new DataDealerRgoodsStatsWorker("dataDealerRgoodsStatsWorker","年进货量统计");
