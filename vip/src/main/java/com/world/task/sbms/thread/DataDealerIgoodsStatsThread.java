 ...
 */
public class DataDealerIgoodsStatsThread extends Thread{
    private static Logger logger = Logger.getLogger(DataDealerIgoodsStatsThread.class);
    private List<DealerGoodsStats> dealerGoodsStatsList;
    private CountDownLatch countDownLatch;

    public DataDealerIgoodsStatsThread(List<DealerGoodsStats> dealerGoodsStatsList, CountDownLatch countDownLatch) {
    ...
    public void run() {
        ...
        try{
            //1、插入数据
            String sql = "insert into data_dealer_igoods_stats (dealer_code,dealer_name,province_code,province_name," +
                    "brand_code,brand_name,YEAR,in_goods_quantity_m1,in_goods_quantity_m2,in_goods_quantity_m3," +
                    "in_goods_quantity_m4,in_goods_quantity_m5,in_goods_quantity_m6,in_goods_quantity_m7,in_goods_quantity_m8," +
                    "in_goods_quantity_m9,in_goods_quantity_m10,in_goods_quantity_m11,in_goods_quantity_m12,create_date) " +
                    " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            param.add(dealerGoodsStatsList.get(0).getBrandName());
            param.add(BrandEnum.getByCode(dealerGoodsStatsList.get(0).getBrandName()).getName());
            param.add(year);
