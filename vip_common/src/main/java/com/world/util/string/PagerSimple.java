...
//		}catch(Exception ex){}
		...
		if(curpage == 1){
			...
			bar.append("<a class=\"Pbtn first\" href=\""+urlstart+"1"+urlend+"\">第一页</a>").append("<a class=\"Pbtn pre\" href=\""+urlstart+(curpage-1)+urlend+"\"><i>&lt;</i> 上一页</a>");
		}
		/*
		 * 前面部分
		 */ 
		...
		//中间部分
		bar.append("<a class='num current'>"+curpage+"</a>");
		//后面部分
		if(getPages()>(curpage+showItems+1))
		{
		  for(int i=(curpage+1);i<(curpage+showItems+1);i++){
		...
		if(curpage == pages){
			...
			bar.append("<a class=\"Pbtn next\" href=\""+urlstart+(curpage+1)+urlend+"\">下一页 <i>&gt;</i></a>");
		}
		//bar.append("</div>");
		//如果现实搜索 
		if(isShowSearch)
			bar.append("<div class=\"go_page\"><span>到第</span><input type=\"text\" id=\"PagerInput\" size=2 maxSize=\""+getPages()+"\" mytitle=\"请输入一个1至"+getPages()+"之间的页码,然后回车或者点击跳转进行跳转\"  pattern=\"num()\" errmsg=\"请检查您输入的是否为一个1至"+getPages()+"之间的页码 ！\"  value=\""+curpage+"\"/><span>页</span><a href=\"javascript:void(0)\" id=\"JumpButton\" class=\"Pbtn jump\">跳转</a></div>");
		return bar.toString();
	}
