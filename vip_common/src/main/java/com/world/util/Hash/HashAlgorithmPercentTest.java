...
public class HashAlgorithmPercentTest {
	...
	private final static Logger log = Logger.getLogger(HashAlgorithmPercentTest.class);
	...
	public static void main(String[] args) {
		...
		log.info("Normal case : nodes count : " + allNodes.size());
		call(allNodes, map);
		
		allNodes = getNodes(NODE_COUNT + 8);
		log.info("Added case : nodes count : " + allNodes.size());
		call(allNodes, map);
		
		allNodes = getNodes(NODE_COUNT - 10);
		log.info("Reduced case : nodes count : " + allNodes.size());
		call(allNodes, map);
		...
		for (Map.Entry<String, List<Node>> entry : map.entrySet()) {
			...
			if (list.size() == 3) {
				...
				log.info("It's wrong size of list, key is " + entry.getKey() + ", size is " + list.size());
			}
		}
		
		log.info(addCount + "   ---   " + reduceCount);
		
		log.info("Same percent in added case : " + (float) addCount * 100 / EXE_TIMES + "%");
		log.info("Same percent in reduced case : " + (float) reduceCount * 100 / EXE_TIMES + "%");
