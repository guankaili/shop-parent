 ...
 **/
public class WorkDataShopScanInMYStatsWorker extends Worker {

    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public WorkDataShopScanInMYStatsWorker(String name, String des) {
        super(name, des);
    }
    ...
    public void run() {
        ...
        if (workFlag) {
            ...
            try {
                ...
                List<Bean> shopBeans = Data.Query("scan_main", sqlScan, null, WorkDataShopScanInMYStats.class);
                if (StringUtil.isNotEmpty(shopBeans)) {
                    List<WorkDataShopScanInMYStats> scanList = ObjectConversion.copy(shopBeans, WorkDataShopScanInMYStats.class);
                    List<WorkDataShopScanInMYFromStats> list = fromData(scanList);
                    //构建线程池
                    //当提交的任务数量为1000的时候，会开辟20个线程数
                    ExecutorService executorService = Executors.newFixedThreadPool(10);
                    CountDownLatch countDownLatch = new CountDownLatch(list.size());
                    for(WorkDataShopScanInMYFromStats workDataShopScanInMYFromStats : list){
                        //业务处理线程
                        WorkDataShopScanInMYStatsThread workDataShopScanInMYStatsThread = new WorkDataShopScanInMYStatsThread(workDataShopScanInMYFromStats,countDownLatch);
                        executorService.submit(workDataShopScanInMYStatsThread);
                    }
     ...
     * @return
     */
    private List<WorkDataShopScanInMYFromStats> fromData(List<WorkDataShopScanInMYStats> scanList) {
        List<WorkDataShopScanInMYFromStats> ins = new ArrayList<WorkDataShopScanInMYFromStats>();
        for (WorkDataShopScanInMYStats bean : scanList) {
            WorkDataShopScanInMYFromStats fromBean = new WorkDataShopScanInMYFromStats();
    ...
    public static void main(String[] args) {
        WorkDataShopScanInMYStatsWorker workDataShopScanInMYStatsWorker = new WorkDataShopScanInMYStatsWorker("WorkDataShopScanInMYStatsWorker", "门店扫码入库柱状图数据同步 数据同步");
        workDataShopScanInMYStatsWorker.run();
    }
