...
import com.world.task.sbms.thread.WorkDataShopScanInDetailDStatsThread;
 ...
 **/
public class WorkDataShopScanInDetailDStatsWorker extends Worker {

    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public WorkDataShopScanInDetailDStatsWorker(String name, String des) {
        super(name, des);
    }
        ...
        if (workFlag) {
            ...
            try {
                String sqlIn = " SELECT" +
                        ...
                        "GROUP BY " +
                        "t.dealer_cm_id ";
                List<Bean> shopBeans = Data.Query("scan_main", sqlIn, null, WorkDataShopScanInDetailDStats.class);
                if (StringUtil.isNotEmpty(shopBeans)) {
                    List<WorkDataShopScanInDetailDStats> inDetailDStats = ObjectConversion.copy(shopBeans, WorkDataShopScanInDetailDStats.class);
                    ...
                    //查询店铺数量
                    List<Bean> ScanBeans = Data.Query("shop_member", sqlShop, null, WorkDataShopScanInDetailDStats.class);
                    if (StringUtil.isNotEmpty(ScanBeans)) {
                        List<WorkDataShopScanInDetailDStats> inDetailDStatsTow = ObjectConversion.copy(shopBeans, WorkDataShopScanInDetailDStats.class);
                        ...
                        inDetailDStats.forEach(item1 -> {
                            inDetailDStatsTow.forEach(item2 -> {
                                if (item1.getDealerCmId().equals(item2.getDealerCmId())) {
                                    ...
                                    //计算参与率
                                    item1.setShopJoinInRate(accuracy(item2.getShopJoinInQuantity(), item2.getShopSignQuantity(), 3));
                                }
                            });
                        });
                        //构建线程池
                        ...
                        for (WorkDataShopScanInDetailDStats workDataShopScanInDetailDStats : inDetailDStats) {
                            //业务处理线程
                            WorkDataShopScanInDetailDStatsThread workDataShopScanInDetailDStatsThread = new WorkDataShopScanInDetailDStatsThread(workDataShopScanInDetailDStats, countDownLatch);
                            executorService.submit(workDataShopScanInDetailDStatsThread);
                        }
    ...
    public static void main(String[] args) {
        WorkDataShopScanInDetailDStatsWorker workDataShopScanInDetailDStatsWorker = new WorkDataShopScanInDetailDStatsWorker("workDataShopScanInDetailDStatsWorker", "门店扫码入库柱状图数据同步 数据同步");
        workDataShopScanInDetailDStatsWorker.run();
    }
