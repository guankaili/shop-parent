 ...
 **/
public class WorkDataShopAddAStatsWorker extends Worker {

    /*此轮定时任务结束标识*/
    private static boolean workFlag = true;

    public WorkDataShopAddAStatsWorker(String name, String des) {
        super(name, des);
    }
    ...
    public void run() {
        ...
        if (workFlag) {
            try {
                ...
                List<Bean> beans = Data.Query("shop_member", sqlScanAdd, null, WorkDataShopAddAStats.class);
                if (StringUtil.isNotEmpty(beans)){
                    List<WorkDataShopAddAStats> scanList = ObjectConversion.copy(beans, WorkDataShopAddAStats.class);
                    List<WorkDataShopAddAFromStats> list = fromData(scanList);

                    //构建线程池
                    //当提交的任务数量为1000的时候，会开辟20个线程数
                    ExecutorService executorService = Executors.newFixedThreadPool(10);
                    CountDownLatch countDownLatch = new CountDownLatch(list.size());
                    for(WorkDataShopAddAFromStats workDataShopAddAFromStats : list){
                        //业务处理线程
                        WorkDataShopAddAStatsThread workDataShopAddAStatsThread = new WorkDataShopAddAStatsThread(workDataShopAddAFromStats,countDownLatch);
                        executorService.submit(workDataShopAddAStatsThread);
                    }
     ...
     * @return
     */
    private List<WorkDataShopAddAFromStats> fromData(List<WorkDataShopAddAStats> scanList) {
        List<WorkDataShopAddAFromStats> add = new ArrayList<WorkDataShopAddAFromStats>();
        for (WorkDataShopAddAStats bean : scanList) {
            WorkDataShopAddAFromStats fromBean = new WorkDataShopAddAFromStats();
    ...
    public static void main(String[] args) {
        WorkDataShopAddAStatsWorker workDataShopAddAStatsWorker = new WorkDataShopAddAStatsWorker("workDataShopAddAStatsThread", "门店统计 新增门店统计 数据同步");
        workDataShopAddAStatsWorker.run();
    }
