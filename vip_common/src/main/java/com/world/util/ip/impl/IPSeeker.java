...
public class IPSeeker {
	...
	private static Logger log = Logger.getLogger(IPSeeker.class.getName());

	//纯真IP数据库名
	private String IP_FILE="/data/addressdb/qqwry.dat";
	//保存的文件夹
	...
	public void saveIPEntries() {
	    try {
            ...
            IPEntryMongoDao dao = new IPEntryMongoDao();
		    int endOffset = (int)ipEnd;
            for(int offset = (int)ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
                try {
					int temp = readInt3(offset);
					if(temp != -1) {
					    IPLocation ipLoc = getIPLocation(temp);
					    // 判断是否这个地点里面包含了s子串，如果包含了，添加这个记录到List中，如果没有，继续
					    IPEntryMongo entry = new IPEntryMongo();
					    entry.country = ipLoc.getCountry();
					    entry.area = ipLoc.getArea();
					    // 得到起始IP
					    readIP(offset - 4, b4);
